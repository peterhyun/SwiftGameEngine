...
//
AudioSystem::AudioSystem( AudioSystemConfig const& audioSystemConfig)
	: m_audioSystemConfig(audioSystemConfig)
...
SoundID AudioSystem::CreateOrGetSound( const std::string& soundFilePath, bool isSound3D)
{
	...
	else
	{
		...
		m_fmodSystem->createSound( soundFilePath.c_str(), isSound3D? FMOD_3D : FMOD_DEFAULT, nullptr, &newSound );
		if( newSound )
		{
...
{
	FMOD_VECTOR lstnrPos = {-listenerPosition.y, listenerPosition.z, -listenerPosition.x};
	FMOD_VECTOR lstnrVel = { 0.0f, 0.0f, 0.0f };
	FMOD_VECTOR lstnrFwd = {-listenerForward.y, listenerForward.z, -listenerForward.x};
	FMOD_VECTOR lstnrUp = {-listenerUp.y, listenerUp.z, -listenerUp.x};
	m_fmodSystem->set3DListenerAttributes(listenerIndex, &lstnrPos, &lstnrVel, &lstnrFwd, &lstnrUp);
}
...
SoundPlaybackID AudioSystem::StartSoundAt(SoundID soundID, const Vec3& soundPosition, bool isLooped, float volume, float balance, float speed, bool isPaused)
{
	...
	if (channelAssignedToSound)
	{
		...
		FMOD_VECTOR soundPos = { -soundPosition.y, soundPosition.z, -soundPosition.x };
		FMOD_VECTOR soundVel = { 0.0f, 0.0f, 0.0f };
		channelAssignedToSound->set3DAttributes(&soundPos, &soundVel);
	};
